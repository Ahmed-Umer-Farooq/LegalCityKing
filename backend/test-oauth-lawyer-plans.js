      ...
      oauthLawyers.forEach(lawyer => {
        ...
        if (hasRestrictions) {
          try {
            ...
            const expectedRestrictions = getPlanRestrictions(tier);
            const hasCorrectStructure = Object.keys(expectedRestrictions).every(key => 
              restrictions.hasOwnProperty(key)
            );
            console.log(`     - Structure: ${hasCorrectStructure ? '✅ Correct' : '❌ Incorrect'}`);
          } catch (e) {
            console.log(`     - Structure: ❌ Invalid JSON`);
          }
        }
      });
    }
