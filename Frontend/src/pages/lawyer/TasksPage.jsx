  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-[#181A2A]">Tasks</h1>
          <p className="text-[#737791] mt-1 text-sm sm:text-base">Manage your tasks and deadlines</p>
        </div>
        <button
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filteredTasks.map((task) => (
                <div key={task.id} className="border border-[#F8F9FA] rounded-lg p-3 sm:p-4 hover:shadow-md transition-shadow">
                  <div className="flex flex-col sm:flex-row items-start justify-between gap-3 mb-3">
                    <div className="flex-1 w-full">
                      <h3 className="font-semibold text-[#181A2A] mb-1 text-sm sm:text-base">{task.title}</h3>
                      <p className="text-xs sm:text-sm text-[#737791] mb-2">{task.description}</p>
                      <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-[#737791]">
                    ...
                    <div className="flex sm:flex-col gap-2 w-full sm:w-auto sm:ml-4">
                      <span className={`px-2 py-1 text-xs rounded-full ${getPriorityColor(task.priority)} flex-1 sm:flex-initial text-center`}>
                        {task.priority?.charAt(0).toUpperCase() + task.priority?.slice(1)}
                      </span>
                      <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(task.status)} flex-1 sm:flex-initial text-center`}>
